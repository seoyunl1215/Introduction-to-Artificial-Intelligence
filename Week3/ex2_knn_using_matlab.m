...
% 학습데이터. 71~100: versicolor, 121~150: verginica 총 60개
% 평가데이터. 51~70: versicolor, 101~120: verginica 총 40개

% 일단, 사용할 특징은 sepal length와 width = 1열과 2열
tr_id = [71:1:100 121:1:150];
Trainging_data = meas(tr_id, 1:2);
Trainging_label = spcs2num(tr_id, :);

ts_id = [51:1:70 101:1:120];
Test_data = meas(ts_id, 1:2);
Test_label = spcs2num(ts_id, :);

%% 매트랩 내부 함수를 이용한 KNN 모델 만들기 (학습: 결정해야 할 거시 k, 거리를 어떤 방법으로 할지)
k = 3; % 인접한 이웃 3개를 보겠다.
mdl = fitcknn(Trainging_data, Trainging_label, "NumNeighbors", k, "Distance", "euclidean");
% mdl = fitcknn(Trainging_data, Trainging_label, "NumNeighbors", k); % knn
% 에서 거리를 계산하는 default 방법은 유클리디안 이므로, 이렇게 작성해도 됨
